...
REQUIRED_VARS="DB_WRITER_HOST DB_PORT DB_NAME DB_APP_USERNAME DB_APP_PASSWORD"
...
for i in $(seq 1 30); do
  if mysqladmin ping -h"$DB_WRITER_HOST" -P"$DB_PORT" -u"$DB_APP_USERNAME" -p"$DB_APP_PASSWORD" --silent; then
...
# 30回試してもRDS/Aurora MySQLに接続できなかった場合は異常終了
if ! mysqladmin ping -h"$DB_WRITER_HOST" -P"$DB_PORT" -u"$DB_APP_USERNAME" -p"$DB_APP_PASSWORD" --silent; then
...
mysql -h"$DB_WRITER_HOST" -P"$DB_PORT" -u"$DB_APP_USERNAME" -p"$DB_APP_PASSWORD" "$DB_NAME" < /app/initdata.sql
