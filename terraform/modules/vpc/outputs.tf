...
output "vpc_private_subnet_ids" {
  description = "The ID of the private subnet (always get the ID of the private subnet that changes dynamically according to the AZ and NAT configuration. Used in ECS module, etc.)"
  value       = { for k, v in aws_subnet.terra_subnet : substr(k, 8, 2) => v.id if substr(k, 0, 7) == "private" }
}

output "vpc_protected_subnet_ids" {
  description = "The ID of the protected subnet (always get the ID of the protected subnet that changes dynamically according to the AZ and NAT configuration. Used in ECS module, etc.)"
